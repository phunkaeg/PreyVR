...
- `PreyVR.dll` version `0.3.0-lifecycle-hardening` checks 22 exact in-memory landmarks and packages a default-off `EndRendererScene` observer. The current binary passes the fresh 11-test headless loop and the installed-module gate, but has not yet been loaded into Prey. The earlier 21-landmark `0.2.0` artifact counted 301 live callbacks; that historical run does not prove this rebuilt DLL.
