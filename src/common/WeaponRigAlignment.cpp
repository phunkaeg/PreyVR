...
constexpr std::uintptr_t kSkinVtable = 0x1D1EDA8;
constexpr std::uintptr_t kWrenchVtable = 0x1E92F00;
constexpr std::uintptr_t kDisruptorVtable = 0x1E31290;
...
struct Reader {
    ...
    bool PoseRotation(std::uintptr_t array, unsigned count, int index, Quaternion& q) const {
        return index >= 0 && count<=kMaxJoints &&
            static_cast<unsigned>(index) < count && Rotation(array, index * 0x1Cull, q);
    }
    bool BindRotation(std::uintptr_t skeleton, int index, Quaternion& q) const {
        // Concrete embedded pose vtable 0x1D27228, +0x48 -> 0x87C6D0:
        // return *(this+0x18) + index*28. This is a raw slice, NOT a DynArray.
        // Count lives at embedded pose +8 (skeleton+0x20), not slice[-4].
        std::uintptr_t array=0;
        unsigned count=0;
        return index>=0 && Get(skeleton,0x20,count) && count<=kMaxJoints &&
            static_cast<unsigned>(index)<count && Get(skeleton,0x30,array) &&
            Rotation(array,index*0x1Cull,q);
    }
};
                 ...
                 unsigned absoluteCount, int wristJoint, Basis& out)
{
    ...
    if (!r.PoseRotation(absolutePose, absoluteCount, wristJoint, wrist) ||
        !r.PoseRotation(absolutePose, absoluteCount, candidate.socketJoint, socket) ||
        !r.BindRotation(skeleton, candidate.socketJoint, bind) ||
    ...
    if (!r.Is(weaponManager, moduleBase + kManagerVtable) ||
        ...
        !r.Get(owner.weapon, 0x2F0, helperName)) {
        return Status::missingHelper;
    }
    // Live Steam wrench: concrete vtable 0x1E92F00, empty ammo helper,
    // identity root/cog bind rotations. It has no barrel to reconcile. Align
    // its authored model frame to the controller instead of capturing an
    // arbitrary equip angle, or refusing all positional hand motion.
    // This is an explicit melee presentation policy, not a fabricated muzzle.
    char first=0;
    if (r.Is(owner.weapon,moduleBase+kWrenchVtable) && r.Get(helperName,0,first) && !first) {
        candidate.barrelInWeapon = {};
        candidate.source = Source::wrenchModel;
        out=candidate;
        return Status::ready;
    }
    if (!r.String(helperName,candidate.helper)) { return Status::missingHelper; }
    // The Disruptor's positional "muzzle" is authored with +Y along +Z of
    // the visible weapon. Its fx_muzzle has the barrel-facing frame instead
    // (both default and published transforms measured in the loaded save).
    // Asset-specific presentation policy; the ammo position/name stays native.
    if(r.Is(owner.weapon,moduleBase+kDisruptorVtable)) {
        std::memcpy(candidate.helper,"fx_muzzle",sizeof("fx_muzzle"));
    }
    ...
    for (unsigned i = 0; i < helperCount; ++i) {
        ...
        char name[128]{};
        if (!r.Get(helpers, i * 8ull, helper)) { return Status::unsupportedHelper; }
        const bool bone=r.Is(helper,moduleBase+kBoneVtable);
        const bool skin=r.Is(helper,moduleBase+kSkinVtable);
        // Concrete +0x10 virtual getters: bone 0x8CE820 -> [this+0x10],
        // skin 0xE92610 -> [this+0x18]. An unrelated skin is not a refusal;
        // a matching skin still cannot supply the bone formula below.
        if ((!bone && !skin) || !r.Get(helper,skin?0x18:0x10,namePointer) ||
            !r.String(namePointer, name)) {
            return Status::unsupportedHelper;
        }
        if (!EqualName(candidate.helper, name)) { continue; }
        if (!bone) { return Status::unsupportedHelper; }
    ...
    for (unsigned i = 0; i < weaponCount; ++i) {
        ...
        if (!r.Get(weaponSkeleton, 0x30, weaponBind) ||
                !r.BindRotation(weaponSkeleton, static_cast<int>(i), candidate.barrelInWeapon)) {
            return Status::invalidBasis;
        }
...
const char* SourceName(Source source)
{
    ...
    case Source::wrenchModel: return "wrench_model";
    default: return "none";
    }
}
