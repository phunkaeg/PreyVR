...
param(
    ...
    [switch]$NoHands,
    # Opt-in authored barrel alignment for supported character-bound weapons.
    # Requires -Controls and hands; live asset/recoil coverage is still pending.
    [switch]$WeaponAimAlignment,
...
if ($WeaponAimAlignment -and (-not $Controls -or $NoHands)) {
    throw '-WeaponAimAlignment requires -Controls with hands enabled.'
}
...
# Reapply after RT_BeginFrame returns, on the latch's own thread (R-069).
# This survives subsequent cvar resets. The console can still report a different
# cvar value; near.fov reports the last native latch and the override separately.
...
if ($Controls -or $Reticle) {
    ...
    if ($Hands) {
        ...
        Step 'weapon aim alignment' $(if ($WeaponAimAlignment) { 'ik.align 1' } else { 'ik.align 0' })
        ...
        Step 'weapon alignment status' 'ik.align'
        # Reported rather than assumed: `ik.calibrate` returning 0 means the
        # request was accepted, never that it was captured. With authored
        # alignment only the LEFT hand calibrates; ikCalR=0 is expected.
        Start-Sleep -Milliseconds 800
        $ik = Send-Cmd 'report'
        $flags = ($ik -split ' ' | Where-Object { $_ -match '^ik(CalR|CalL)=' }) -join ' '
        $captured = ($flags -match 'ikCalL=1') -and ($WeaponAimAlignment -or ($flags -match 'ikCalR=1'))
        if ($captured) {
        ...
        if ($WeaponAimAlignment) {
            Write-Host ('  right weapon alignment: ' + (Send-Cmd 'ik.align'))
            Write-Host '  Right alignment uses the authored barrel basis; ikCalR=0 is expected.'
        }
    }
}
