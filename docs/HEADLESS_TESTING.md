...
| `engine_map` | All 28 promoted runtime signatures match a synthetic supported image; a one-byte mutation and truncated image both fail closed. It also pins the ArkPlayer interaction/selector/usable-entity offsets and the `0x70`-byte candidate-record contract. |
...
| `frame_dump` | The dump header round-trips and fails closed on a bad magic, unknown version, zero or oversized dimensions, a row pitch too small for the width, and a truncated or overlong payload. Comparison is exact on a known single-channel change, ignores alpha, survives differing row pitches, and reports mismatched frames as incomparable rather than as a zero difference. |
| `console_policy` | The console allowlist denies unknown commands, `exec` by name and in any casing, semicolon/newline/carriage-return/NUL command chaining, quotes and path separators, multiple or non-numeric arguments, and overlong lines. Allowlisted names match case-insensitively because CryEngine does. |
...
On 2026-08-23 the fresh Release loop passed 12/12 tests, the twelfth being the runtime-snapshot fixture. With the researched game installation available, the deduplicated build doctor reports 7 passes, 0 warnings, and 0 failures; the single landmark result covers all 30 compiled entries. Builds are reproducible since `/Brepro` was added, so the same source yields the same artifact hash — verified across two forced relinks. If the game directory is absent and not required, it emits an explicit warning and still tests the portable parts. The smoke-host log is redirected into the CTest temporary directory, so the headless loop does not overwrite a real in-game smoke log.
