...
- Prefer a verified engine scene-render/re-entry/multi-view seam over indiscriminate draw duplication. **The static preconditions for native stereo are met** (H-009): `C3DEngine::RenderWorld` is virtual at `IProcess` slot 3 with zero direct call sites, and `CreateGeneralPassRenderingInfo` is a callable function taking an arbitrary `CCamera&`, making `RenderWorld(flags, CreateGeneralPassRenderingInfo(eyeCamera, ...), "EyeL")` expressible. Viability is **not** established: nothing has been called, render-target and pooled-`CRenderView` behaviour under a second pass is unknown, and cost is unmeasured.
- The global view camera is read from **at least 84 call sites** across rendering, gameplay, UI and the 3D engine — not from a handful. Any design that borrows `CSystem::m_ViewCamera` must assume the engine keeps observing it.
