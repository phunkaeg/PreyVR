...
**Written 2026-09-07, for the run on 2026-09-08.** Build `b2d9a2d`,
`PreyVR.dll` version `0.3.1-static-ik-integration`, already built in both trees.
...
D:\Dev` Debug\PreyVR\tools\Invoke-PreyVRIkTest.ps1 -Drive               # controller
```
...
| `camYawMdeg` / `headYawMdeg` / `playYawMdeg` | `play = cam - head` | if `play` moves when only the head turns, the yaw fix failed |
