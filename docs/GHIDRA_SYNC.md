...
| 2026-09-06 | `/Prey/PreyDll.dll` | Steam `7D6E322F…B05311A7` | Named four functions and wrote plate comments carrying this session's findings | `0x81D0D0` → `CCharInstance_RenderCHR`, with the per-frame matrix copy (R-095) and the near predicate (R-089/R-093) recorded on it. `0x9D7430` → `CBaseInputSendEventToListeners`, with the four-stage walk and the fact that the title consumer sits at the **exclusive** stage, not the normal list. `0x9D6D30` → `CBaseInputDispatchInputEventGated`, carrying the true symbol `CBaseInput::PostInputEvent`, the full `SInputEvent` layout and R-091's live acceptance receipt. `0x81BCB0` → `CCharInstanceRenderVirtual`, carrying H-018 Gap 1. `0x9D7790` was already created and named `Prey_CBaseInput_IsInputBlocked` by the H-013 investigation — left as found. Program saved. |
