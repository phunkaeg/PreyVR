...
struct Fixture {
    ...
    Status Resolve(Basis& b) { return ReadBasis({this, Read}, base, owner, pose, 2, b); }
    Fixture() {
...
void LookupAndRefusals()
{
    ...
    refusal([](Fixture& x) { x.Put(x.pose-4, 0, std::uint32_t(2)); }, Status::invalidMount, "truncated current pose");
    ...
    Check(ReadBasis({}, Fixture::base, f.owner, f.pose, 2, b) == Status::invalidOwner,
