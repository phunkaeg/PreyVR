...
| R-132 | `PreyDll.dll` | `DAT_182B3F778` (`$ZTarget`), `DAT_182B3F738` (`$SceneTarget`), `DAT_182B3F688` (`$BackBuffer`); creator `FUN_180F564B0`; `CTexture+0x30` device texture, `+0x54` flags | CryEngine's scene render targets, and the scene DEPTH we actually want | The creator builds all three and re-sizes them in place when they exist; `$ZTarget` is created with flags `0x51000` and the same value is written to `+0x54` on the resize path. `+0x30` is the device texture, established by the engine's own guard `if (tex == 0 || *(longlong*)(tex + 0x30) == 0)` | Static decompile 2026-09-10. Luma independently asserts `CTexture::m_nFlags` at `+0x54` on the same build, matching the `+0x54 = 0x51000` write here | verified | **Supersedes `+0x9970` for depth submission** (R-131): that is the swapchain's native Z surface, while a deferred renderer writes scene depth here. One hop remains -- `CDeviceTexture` to `ID3D11Texture2D` -- left for a runtime check, where a wrong offset fails `QueryInterface` visibly instead of yielding plausible wrong pixels. `$ZTargetScaled` also exists (3 refs); which the compositor should receive is undecided. |
| R-130 | `PreyDll.dll` | `CD3D9Renderer+0x9970` (DSV), `+0xAE00` (RTV), `+0xAE88` (swapchain); call site `0x180F38F12` in `FUN_180F38060` | `m_pNativeZSurface`, `m_pBackBuffer` and our swapchain field, **all three verified at one call** | The function loads `[RSI+0xAE88]`, calls it for the current buffer index, asserts `[RSI+0xAE00]` equals the swapchain buffer at that index, then issues `OMSetRenderTargets(1, &{[RSI+0xAE00]}, [RSI+0x9970])` — vtable `+0x108` is index 33 of `ID3D11DeviceContext`, and Windows x64 puts `pDepthStencilView` in R9, which is loaded from `+0x9970` | Static decompile and disassembly 2026-09-10 | verified | **Depth is now reachable.** `XR_KHR_composition_layer_depth` takes a depth image per submitted eye and improves runtime reprojection, which this build leans on at ~78 fps against 90 Hz. Two cautions: this DSV is bound alongside the BACKBUFFER so it is the final target rather than necessarily the scene depth a compositor wants; and a DSV is not an image — the texture behind it must be obtained and its format checked. Supersedes the R-129 note that `+0x9970` was unverified, and withdraws the inference that it belonged to a `0x28`-stride array. |
| R-129 | `PreyDll.dll` | `CD3D9Renderer+0xD70`/`+0xD74`; consumer `FUN_180F42C00`; producer `RT_RenderScene` `0x180F42299` | `m_vProjMatrixSubPixoffset`, the projection sub-pixel jitter, **verified** | `FUN_180F42C00` adds `+0xD70`/`+0xD74` into the floats at `+0x190`/`+0x194`, elements 8 and 9 of the 4x4 matrix beginning at `+0x170` — the third row's first two components, where a sub-pixel offset enters a projection matrix. Producer writes it in `RT_RenderScene` (`MOVSS [RBP+0xd70],XMM2`); consumer reads it (`MOVSS XMM0,[RDI+0xd70]`). The add is gated by bit `0x1B` of `+0x5484` and a byte at `frameBlock+0x49A2` | Static decompile 2026-09-10. **The same function independently confirms our R-026**: it computes `*(uint*)(this+0x499C) * 0x328`, our `frameSlotIndex` and `frameBlockStride`, in code this project had never opened | verified | **Proves Luma's map and ours describe ONE struct**, not merely two non-conflicting maps (R-128). The same receiver carries our `+0x499C` and their `+0xD70`. **VR consequence:** jitter is applied per frame and this mod alternates eyes per frame, so the sequence is split between eyes — each sees every other sample. A concrete, previously unrecorded candidate for stereo artefacts, consistent with the displaced weapon ghost that led this project to abandon temporal AA. Untested: nobody has pinned `+0xD70` to zero and looked. Luma's `m_pNativeZSurface` (`+0x9970`) is **not** verified — every `[reg+0x9970]` hit is float stack traffic, and a DSV pointer cannot be a float. |
| R-128 | `PreyDll.dll` | `CD3D9Renderer` `+0xD70` `+0x9898` `+0x9970` `+0xAE00` `+0xAF38` `+0xAF80`; `RT_RenderScene` RVA `0xF41CA0` | Renderer fields and the render-scene entry, from the Luma framework's Prey plugin | Luma discriminates builds by PE `TimeDateStamp` and lists six; our `PreyDll.dll` reads `0x5D1CB240` = their `PreySteam`, so their column applies to our target directly. Their offsets are `static_assert`-checked in their own build. Reconciled against our `RendererLayout`: **zero conflicts** — every field of ours lands in a region they mark unknown and vice versa | Build identity verified here by reading the PE header. `RT_RenderScene` verified to land in `.text` with an x64 prologue (`44 89 44 24 18 48 89 54 24 10 53 55`). The struct offsets are **not** independently verified | external-lead | **Recorded as leads and deliberately NOT wired into `EngineMap.h`.** Another project's assertions; proven build identity removes the transfer problem, not the need to verify before shipping a write. Highest value: `m_pNativeZSurface` (`+0x9970`) would give us depth, which we have never had and which `XR_KHR_composition_layer_depth` wants; `m_vProjMatrixSubPixoffset` (`+0xD70`) is projection jitter, and per-frame jitter on an alternating-eye scheme is a candidate for artefacts attributed elsewhere. Luma is **Custom MIT** — credit required, commercial use needs permission — so no code may be absorbed into this MIT project. Both hook D3D11; do not assume they coexist. |
...
| RVA | name | role |
|---|---|---|
| `0x877600` | `Prey_AnimCommandBufferExecute` | animation job: commands (case 9 = layer pose modifiers) -> FK -> ADIK -> post modifiers -> proxies |
| `0x8797C0` | `Prey_CommandPoseModifierExecute` | layer-pushed modifier `Execute`, inside the command buffer |
...
| `0x7C4C60` | `Prey_CAttachmentBONESimulateEntityBinding` | spring/pendulum on the binding; `+0x28` enabled, `+0x2B` redirect writes back into the pose (H-020 candidate) |
| `0x82EE10` (`0x82E580`) | `Prey_SkinningTransformationsComputation` | reads `pose+0x18` absolute directly; render side, after attachments |
| `0x2C30D0` | `Prey_CryCreateClassInstance` | factory by class name -> `{interface, control block}` |
| `0x1807DD2A0` / `0x1807DD0A0` | `Prey_COperatorQueueExecute` / `...PushPosition` | vtable `0x181D1F6C0`: `+0x40` PushPosition, `+0x48` PushOrientation, `+0x70` Clear; EOp 0 abs / 1 rel / 2 world / 3 add / 4 add-rel |
